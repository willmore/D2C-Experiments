...
def main(argv=None):
    ...
    numHosts = 8
    script = "/home/dirac/amAu111BF4/22vdw.py"
    doneFile = "/tmp/done.txt"
    startCmd = "mpirun -np %d -hostfile /tmp/d2c.context -leave-session-attached gpaw-python %s 2>&1 > experiment.out; date > %s" % (numHosts, script, doneFile)


    instanceType = InstanceType("m1.xlarge", 2, 7500, 850, [])
    fixSSHCmd = "echo -e \"Host *\n   StrictHostKeyChecking no\" > .ssh/config"

    deployment = Deployment("22vdw_py", 
                            ec2ConnFactory, 
                            roles=[
                                   Role("22vdw_py", "master", 
